...
def on_ui_tabs():
    with gr.Blocks(analytics_enabled=False) as openpose_editor:
        with gr.Row():
            with gr.Column():
                width = gr.Slider(
                    label="width", minimum=64, maximum=2048, value=512, step=64, interactive=True)
                height = gr.Slider(
                    label="height", minimum=64, maximum=2048, value=512, step=64, interactive=True)
                with gr.Row():
                ...
                with gr.Row():
                    ...
                    png_input_area = gr.Image(
                        label="Detect from image", elem_id="openpose_editor_input", visible=False)
                    bg_input = gr.Button(value="Add Background image")
                with gr.Row():
                    preset_list = gr.Dropdown(label="Presets", choices=sorted(
                        presets.keys()), interactive=True)
            ...
            with gr.Column():
                ...
                jsonbox = gr.Text(
                    label="json", elem_id="hide_json", visible=False)
                with gr.Row():
                    ...
                    control_net_max_models_num = getattr(
                        opts, 'control_net_max_models_num', 0)
                    select_target_index = gr.Dropdown([str(i) for i in range(
                        control_net_max_models_num)], label="Send to", value="0", interactive=True, visible=(control_net_max_models_num > 1))

        def estimate_all(img):
        ...
        def estimate(img):
            ...
            if hand_estimation is None:
                model_path = os.path.join(
                    models_path, "openpose", "hand_pose_model.pth")
                if not os.path.isfile(model_path):
                    hand_model_path = "https://huggingface.co/lllyasviel/ControlNet/resolve/main/annotator/ckpts/hand_pose_model.pth"
                    load_file_from_url(
                        hand_model_path, model_dir=os.path.join(models_path, "openpose"))
                hand_estimation = Hand(model_path)

            candidate, subset, hand_peaks = estimate_all(pil2cv(img))
            # candidate, subset = body_estimation(pil2cv(img))

            result = {
                "candidate": candidate2li(candidate),
                "subset": subset2li(subset),
                "hand_peaks": hand2li(hand_peaks),
            }
        ...
        width.change(None, [width, height], None,
                     _js="(w, h) => {resizeCanvas(w, h)}")
        height.change(None, [width, height], None,
                      _js="(w, h) => {resizeCanvas(w, h)}")
        ...
        send_t2t.click(None, select_target_index, None,
                       _js="(i) => {sendImage('txt2img', i)}")
        send_i2i.click(None, select_target_index, None,
                       _js="(i) => {sendImage('img2img', i)}")
        ...
        preset_save.click(savePreset, [dummy_component, dummy_component], [
                          preset_list, preset], _js="savePreset")
